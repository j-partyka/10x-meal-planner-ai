...
import type {
  ComputeShoppingListCommand,
  ComputeShoppingListResponse,
} from "@/types";
...
export function ShoppingListView() {
  const { mealPlan, shoppingList, setMealPlanAndList, hydrate } =
    useMealPlanStorage();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const fetchShoppingList = useCallback(async (plan: NonNullable<typeof mealPlan>) => {
    setError(null);
    setLoading(true);
    try {
